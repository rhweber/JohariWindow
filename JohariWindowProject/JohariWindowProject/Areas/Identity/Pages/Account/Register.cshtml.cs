...
namespace JohariWindowProject.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (role == "") { role = SD.AdminGodRole; } //make the first login a manager)
            ...
            if (ModelState.IsValid)
            {
                ...
                if (!await _roleManager.RoleExistsAsync(SD.AdminGodRole))
                {
                    _roleManager.CreateAsync(new IdentityRole(SD.AdminGodRole)).GetAwaiter().GetResult();
                    _roleManager.CreateAsync(new IdentityRole(SD.CustomerRole)).GetAwaiter().GetResult();
                }
                if (result.Succeeded)
                //assign role to the user (from the form radio options available after the first manager is created)
                { 
                    if (role == SD.AdminGodRole)
                    {
                        await _userManager.AddToRoleAsync(user, SD.AdminGodRole);
                    }
                    else
                    {
